...
% This understanding is exclusively shared  and training of them
% requires huge amount of resources. 
% Therefore, developing a qunantitative method to materialize the systematic interaction between articles of WTO agreements
% as a whole can lower the cost of understanding the system and resolve the insufficient legal capacity issue in WTO DSB.
...
Upon these two considerations, this paper uses deep learning. % to effectively extract information from the text and to generalize the member specific strategic citation patterns.
...
trade dispute. % (\textit{See} Figure \ref{fig:design-of-nn} and Figure \ref{fig:def:io:nn}).
...
The network is fitted as to best explain the variance of each article's citabilities that are predicted by the trained deep neural network. % using a machine learning technique \textit{Random Forests} \citep{rf, genie3}.
...
However, previous literatures have been limited to study the interconnection between relatively small number of articles, which are mostly less than 10. % and has not been conducted in a level of entire WTO agreements.
