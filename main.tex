...
The \textit{max token length} was $35,842$ and $20,158$ for factual aspect and legal article respectively.
...
This process can be conducted with a single \textit{Embedding Layer}\footnote{I used Google News \textit{Word2Vec} to initialize this embedding layer. This is a open-source pretrained word vectors proviced by Google which is trained over Google News corpus (billion unique words). It contains 3 million 300 dimensional English word vectors. Because of the memory limit, I had set 400,000 as a maximum number of word vectors to read form the Google Word2vec and this number corresponds to the \textit{|Size of Dictionary|} in the body.
}. %as illustrated in Figure \ref{fig:embedding}. 
...
from the example factual aspect introduced in \hyperref[sub:factual-aspect-example]{Appendix A.1}. 
The neural network find correpondent $k$-dimensional vectors for given ordered tokens in the document and returns ($n_{\text{max token length}}, k$) size of matrix.
This matrix will be fed into the next layer, \textit{Conv1D}.
